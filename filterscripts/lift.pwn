...
public OnObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= Create3DTextLabel("{ffff00}[Выбор этажа]\n{cccccc}Нажмите H", 0xCCCCCCAA, 1784.9822, -1302.0426, z - 0.9, 4.0, 0, 1);
...
stock Elevator_Initialize()
{
	...
	Label_Elevator          = Create3DTextLabel("{ffff00}[Выбор этажа]\n{cccccc}Нажмите H", 0xCCCCCCAA, 1784.9822, -1302.0426, 13.6491, 4.0, 0, 1);
	...
	for(new i; i < sizeof(Obj_FloorDoors); i ++)
	{
		...
		format(string, sizeof(string), "{ffff00}[ %s этаж (консъерж) ]\n{ffffff}Лифт на: {ffff00}%s этаже (консъерж)\n\n{cccccc}H - для вызова", TextNames[i],TextNames[i]);
...
stock ShowElevatorDialog(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_ID, DIALOG_STYLE_LIST, "{BFBBBA}Выберите этаж", string, "Accept", "Cancel");
